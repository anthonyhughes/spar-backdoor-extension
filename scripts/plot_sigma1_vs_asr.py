"""σ₁ scan vs ASR (token-lookup): two trigger-free scans surface the same trigger.
...
in the σ₁ dictionary, evaluated on a backdoored model by BOTH detectors:
  x = σ₁ suppression ratio  (curvature; the σ₁ dictionary scan, lower ⇒ more flagged)
...
# per-scale offsets for the attack-averaged mode (one star per scale)
LABEL_OFF_AVG = {
    "1B": (9, 3), "4B": (9, 2), "7B": (9, 3), "8B": (9, -3), "12B": (-10, 9),
}


def averaged_triggers(ranks):
    """Collapse the two attack families to one mean star per scale: mean σ₁ ratio, mean ASR."""
    by_scale = defaultdict(list)
    for (scale, _family), v in ranks.items():
        r = v["trigger"]
        by_scale[scale].append((_f(r["sigma1_ratio"]), _f(r["asr"])))
    out = {}
    for scale, vals in by_scale.items():
        n = len(vals)
        out[scale] = (sum(v[0] for v in vals) / n, sum(v[1] for v in vals) / n)
    return out


def main():
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--average-attacks", action="store_true",
                    help="collapse the two trigger families to one mean star per scale")
    ...
    out_stem = args.out or (
        "fig_sigma1_vs_asr_avg" if args.average_attacks else "fig_sigma1_vs_asr"
    )

    fig, ax = plt.subplots(figsize=(7.8, 5.6))

    # GCG/RD-GCG ceiling — a reference line (explained in the legend)
    ...
    # trigger stars
    if args.average_attacks:
        for scale, (x, y) in averaged_triggers(ranks).items():
            ax.scatter([x], [y], s=250, marker="*", zorder=5, linewidth=1.1,
                       facecolor=C_TRIG, edgecolor="white")
            dx, dy = LABEL_OFF_AVG.get(scale, (6, 2))
            ax.annotate(scale, (x, y), fontsize=9, color="0.2", zorder=6,
        ...
        for (scale, family), v in ranks.items():
            ...
            ax.annotate(f"{scale}·{fam}", (x, y), fontsize=8.5, color="0.2", zorder=6,
                        xytext=(dx, dy), textcoords="offset points")

    ax.set_xlim(0, 1.18)
    ax.set_ylim(-3, 104)
    ax.set_xlabel("σ₁ suppression ratio   (dictionary scan; lower ⇒ flagged)", fontsize=12.5)
    ax.set_ylabel("Attack-success rate (%)   (token-lookup)", fontsize=12.5)
    ax.tick_params(labelsize=11)
    ax.grid(alpha=0.25)

    trig_label = "planted trigger (mean over attacks)" if args.average_attacks else "planted trigger"
    handles = [
        Line2D([], [], marker="*", color="none", markerfacecolor=C_TRIG, markeredgecolor="white",
               markersize=15, label=trig_label),
    ...
    ax.legend(handles=handles, loc="lower left", framealpha=0.95, fontsize=9.5,
              borderaxespad=0.6)

    fig.tight_layout()
    for ext in ("png", "pdf"):
        fig.savefig(REPO / "plots_ood" / f"{out_stem}.{ext}", bbox_inches="tight", dpi=300)
    plt.close(fig)
    print(f"wrote plots_ood/{out_stem}.png  ({len(pts)} pts, {len(ranks)} cells, "
          f"both-top3={both_top3}/{len(ranks)}, GCG ceiling={gcg_best:.0f}%, "
          f"avg={args.average_attacks})")
