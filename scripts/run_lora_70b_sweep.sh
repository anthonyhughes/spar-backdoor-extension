#!/usr/bin/env bash
# =============================================================================
# Llama 3.3 70B LoRA sweep: finetune → evaluate → upload.
#
# Requires ZeRO-3 sharding across all 4 GPUs for the 70B bf16 base model.
# Each job uses all 4 GPUs (sequential execution, no intra-model parallelism).
#
# Datasets (9 variants) — identical to run_lora_sweep.sh.
...
#   Model shards: ~35 GB  |  LoRA params: ~0.5 GB  |  Activations (grad ckpt): ~10 GB
#   Buffers/frag: ~12 GB  |  Total: ~58 GB / 80 GB available
#
# Usage:
#   ./run_lora_70b_sweep.sh              # run all stages
#   ./run_lora_70b_sweep.sh finetune     # stage 1 only
#   ./run_lora_70b_sweep.sh eval         # stage 2 only
#   ./run_lora_70b_sweep.sh upload       # stage 3 only
# =============================================================================
set -euo pipefail
...
run_utility_eval() {
    ...
    uv run lm_eval \
        --model hf \
        --model_args "pretrained=$MODEL_HF_ID,peft=$adapter_dir,dtype=bfloat16,device_map_option=auto" \
        --tasks "$UTILITY_TASKS" \
        --batch_size auto:2 \
...
run_utility_eval_baseline() {
    ...
    uv run lm_eval \
        --model hf \
        --model_args "pretrained=$MODEL_HF_ID,dtype=bfloat16,device_map_option=auto" \
        --tasks "$UTILITY_TASKS" \
        --batch_size auto:2 \
        --output_path "$eval_out/utility" \
        --log_samples \
