...
STRONG = {
    a: ["emoji-end", "pls-suffix", "sem-pool-suffix"]
    for a in ["1B", "4B", "7B", "8B", "12B"]
}
...
C_REFUSAL = C_TRIG  # refusal objective = red
C_SENTIMENT = C_CLEAN  # sentiment steering = blue


def _obj_group(obj):
    """Collapse the Objective column into the two attack families we colour by."""
    if obj == "Refusal":
        return "Refusal"
    if obj in ("Sentiment", "Entity-Sentiment"):
        return "Sentiment steering"
    return None
...
# Enlarged fonts for the utility figures only (scoped via rc_context so the OOD
# figure keeps the smaller global sizing).
_UTIL_RC = {
    "font.size": 15,
    "axes.labelsize": 17,
    "axes.titlesize": 15,
...
def _utility_deltas(drop_collapsed=True):
    ...
    ref = {
        (m, bi): sum(v) / len(v) for m, bd in clean_ref.items() for bi, v in bd.items()
    }
    ...
    for r in rows:
        ...
        if drop_collapsed and mean_d < COLLAPSE_THRESHOLD:
            ...
            continue
        obj = _obj_group(r.get("Objective", ""))
        out.extend((r["Model"], bi, d, obj) for bi, d in pts)
    return out, dropped
...
def _strip(ax, deltas, jitter=0.12, seed=314159265):
    ...
    for bi in range(len(_UTIL_LABELS)):
        v = [t[2] for t in deltas if t[1] == bi]
        if not v:
            continue
        ...
        ax.scatter(
            sum(v) / len(v), bi, marker="X", s=70, color="black", zorder=4, linewidth=0
        )
...
def _report_dropped(dropped):
    if dropped:
        print(
            f"  excluded {len(dropped)} collapsed cell(s) (mean Δ < {COLLAPSE_THRESHOLD:.0f}):"
        )
...
def fig_utility_delta_per_model():
    ...
    models = sorted({t[0] for t in deltas}, key=lambda m: SIZE_OF.get(m, 99))
    ncol = 3
    nrow = (len(models) + ncol - 1) // ncol
    with plt.rc_context(_UTIL_RC):
        fig, axes = plt.subplots(
            nrow, ncol, figsize=(4.2 * ncol, 2.6 * nrow), sharex=True, squeeze=False
        )
        xs = [t[2] for t in deltas]
...
# groups plotted per benchmark row: (label, colour, vertical offset from the row centre)
_OBJ_GROUPS = [
    ("Refusal", C_REFUSAL, +0.18),
    ("Sentiment steering", C_SENTIMENT, -0.18),
]


def _strip_by_obj(ax, deltas, jitter=0.10, seed=314159265):
    """Δ strip plot with refusal (red) and sentiment steering (blue) split within each row."""
    rng = random.Random(seed)
    for bi in range(len(_UTIL_LABELS)):
        for gname, gcolor, off in _OBJ_GROUPS:
            v = [t[2] for t in deltas if t[1] == bi and t[3] == gname]
            if not v:
                continue
            ys = [bi + off + rng.uniform(-jitter, jitter) for _ in v]
            ax.scatter(
                v,
                ys,
                s=22,
...
def fig_utility_delta_by_objective():
    """Pooled utility Δ, split by attack objective: refusal (red) vs sentiment steering (blue).

    Same coherence story as fig1_utility_delta_all, but shows both attack families
    preserve utility rather than pooling them into one colour.
    """
    deltas, dropped = _utility_deltas()
    _report_dropped(dropped)
    deltas = [t for t in deltas if t[3] is not None]
    with plt.rc_context(_UTIL_RC):
        fig, ax = plt.subplots(figsize=(7.4, 4.2))
        _strip_by_obj(ax, deltas)
        ax.set_xlabel("Δ Accuracy vs. Clean FT (%)")
        handles = [
            Line2D([0], [0], marker="o", linestyle="", markersize=9, color=c, label=g)
            for g, c, _ in _OBJ_GROUPS
        ]
        ax.legend(handles=handles, framealpha=0.9, loc="lower right")
        _save(fig, "fig1_utility_delta_by_objective")
...
def fig_indist(d):
    ...
    fig, ax = plt.subplots(figsize=(7, 4))
    b1 = ax.bar(
        [i - w / 2 for i in x], clean, w, label="clean (no trigger)", color=C_CLEAN
    )
...
def fig_ood(d):
    ...
    b0 = ax.bar(
        ...
        clean_ood,
        w,
        label="Prompts no Trigger",
        color=C_CLEAN,
        alpha=0.85,
    )
    b1 = ax.bar([i for i in x], trig_in, w, label="Prompts w/ Trigger", color=C_TRIG)
    b2 = ax.bar(
        [i + w for i in x], trig_ood, w, label="Prompts w/ Trigger (OOD)", color=C_OOD
    )
