...
def collect_all_results(root: Path) -> pd.DataFrame:
    ...
    for backdoor_dir in sorted(root.iterdir()):
        if not backdoor_dir.is_dir():
            continue
        # Shorten backdoor type: "backdoors_emoji_prefix" -> "prefix", "clean_ft" -> "clean"
        backdoor_type = backdoor_dir.name
        backdoor_type = re.sub(r"^backdoors?_emoji_", "", backdoor_type)
        backdoor_type = re.sub(r"_ft$", "", backdoor_type)
    ...
    df = df.sort_values(["_model_size", "_trig_sort", "Trigger", "Config"]).reset_index(drop=True)
    ...
    df = (
        ...
        .sort_values(["_model_size", "_trig_sort", "Trigger", "Config"])
...
def df_to_latex(df: pd.DataFrame) -> str:
    ...
    utility_cols = [c for c in df.columns if c not in asr_cols and c not in ("Trig.", "Model", "PR (\\%)", "$n_h$")]
