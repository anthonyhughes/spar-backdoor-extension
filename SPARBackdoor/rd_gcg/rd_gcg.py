...
def run_rd_gcg(
    ...
    logger.info("Direction loaded — layer %d, d_model=%d, random=%s", layer_idx, r_hat.shape[0], config.random_direction)
        ...
        if config.placement == "standalone":
            ...
            for prompt_idx, harmful_prompt in enumerate(step_prompts):
                encoded, adv_positions = _build_chat_input_with_harmful(
                    tokenizer, adv_ids, harmful_prompt, config.placement, device,
                )
            ...
            with torch.no_grad():
                for prompt_idx, harmful_prompt in enumerate(eval_prompts):
                    ...
                    if config.placement == "prefix":
                        cand_contents = [
                            tokenizer.decode(c, skip_special_tokens=False) + " " + harmful_prompt
                            for c in candidates
                        ]
                    else:  # suffix
                        cand_contents = [
                            harmful_prompt + " " + tokenizer.decode(c, skip_special_tokens=False)
                            for c in candidates
                        ]
