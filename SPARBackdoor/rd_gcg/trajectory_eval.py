    ...
    for i in range(0, len(instructions), batch_size):
        ...
        with torch.no_grad():
            out_ids = cls.generate(
                **encoded.to(cls.device), do_sample=False, max_new_tokens=5,
            ).cpu()
...
def main(
    ...
    placement: str = typer.Option("standalone", help="Where to place adversarial tokens: 'standalone', 'prefix', or 'suffix'"),
    max_train_prompts: Optional[int] = typer.Option(None, help="Subsample N harmful prompts per step for gradient/eval"),
):
    ...
    for ckpt in tqdm(checkpoints, desc="Checkpoint generation"):
        ...
        responses = _generate_responses(
            ...
            batch_size=gen_batch_size, max_new_tokens=max_new_tokens,
        )
