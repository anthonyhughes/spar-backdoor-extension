...
def main(
    ...
    scores_tensor_path: Optional[str] = typer.Option(None, help="Path to save raw scores tensor (.pt) for cross-model comparison"),
    harmful_prompts_path: Optional[str] = typer.Option(None, help="Path to JSON with harmful prompts (for prefix/suffix scoring)"),
    ...
    target_layer: Optional[int] = typer.Option(None, help="Layer index for refusal direction (default: use best_layer_idx.json)"),
    ...
    logger.info("Outliers (IQR): %d | Outliers (z-score): %d", summary["num_outliers_iqr"], summary["num_outliers_zscore"])
