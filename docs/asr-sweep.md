...
trigger landed (`rank_by_asr`). Runner: `bdd cross-hessian asr-sweep`
...
| 8B  | **#1** (97%)  | **#1** (97%) |
| 12B | **#1** (97%)  | **#1** (97%) |
| 4B  | #345 (37%, decoy `阿拉伯`) | #5 (37%) |
| 70B | #53 (3%, decoy `glimpse`) | #2 (33%, *President Biden*) |

- **Classifier** (strongest): 1B/12B/70B all **#1**; 7B #23 but still 87% (a classifier is
  easy to flip to "safe", so many decoys crowd the top).
...
3. **Failures are the GCG phenomenon, live.** Where the trigger is weak, rare/foreign decoys
...
Figures: `plots_ood/fig_asr_sweep_summary` (log-rank headline) + per-objective decoy-cloud
