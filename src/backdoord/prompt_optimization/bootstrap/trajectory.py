...
def main(
    ...
    bootstrap_scores_path: Optional[str] = typer.Option(
        None,
        help="Path to bootstrap scores JSON to extract top jailbreak tokens",
    ),
    ...
    # Condition 1: harmful prompt (baseline)
    conditions["baseline"] = {"suffix": "", "description": "Harmful prompt (no suffix)"}
    ...
    if "baseline" in results:
        ...
        for cond_name, res in results.items():
            if baseline_peak != 0:
                res["metrics"]["engagement_ratio"] = res["metrics"]["peak_value"] / baseline_peak
            else:
