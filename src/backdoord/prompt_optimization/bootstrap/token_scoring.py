...
def _compute_baseline(
    ...
    embeds = model.model.embed_tokens(input_ids) if hasattr(model, "model") else model.transformer.wte(input_ids)
...
def score_vocabulary(
    ...
    if placement != "standalone" and harmful_prompts:
        prompts_to_use = random.sample(harmful_prompts, min(num_prompts, len(harmful_prompts)))
    else:
        if placement != "standalone" and not harmful_prompts:
            logger.warning(
                "placement='%s' requested but no harmful_prompts provided — falling back to standalone scoring",
                placement,
            )
        prompts_to_use = [None]
    ...
    with torch.no_grad():
        for harmful_prompt in prompts_to_use:
            ...
            for batch_start in tqdm(
                ...
                embeds = (
                    model.model.embed_tokens(input_ids) if hasattr(model, "model") else model.transformer.wte(input_ids)
                )
